...
def normalize_chapter1_sections(raw_sections: Any) -> tuple[List[Dict[str, Any]], List[str]]:
    ...
    for spec in CHAPTER1_SECTION_SPECS:
        ...
        if key == "industry_supply_chain":
            ...
            paragraphs, section_warnings = _fit_paragraphs_to_slot_count(
                paragraphs,
                slot_count,
                title,
                section_key=key,
...
def _expand_industry_trends_paragraphs(paragraphs: Sequence[str]) -> List[str]:
    expanded: List[str] = []
    for raw in paragraphs:
        text = str(raw or "").strip()
        if not text:
        ...
        for piece in pieces:
            if not merged_piece:
                merged_piece = piece
                continue
            if len(merged_piece) < 10:
                merged_piece += piece
                continue
            expanded.append(merged_piece.strip())
            merged_piece = piece
        if merged_piece:
            expanded.append(merged_piece.strip())
    return _merge_heading_like_paragraphs(expanded)
...
def _fit_paragraphs_to_slot_count(
    ...
    *,
    ...
    if len(fitted) < slot_count:
        ...
        while len(expanded) < slot_count:
            ...
            if idx is None:
                break
            left, right = _split_paragraph_for_template(
                expanded[idx],
                allow_clause_split=section_key == "industry_trends",
            )
        ...
        if section_key == "industry_trends" and len(fitted) < slot_count:
            clause_expanded = _expand_industry_trends_paragraphs(fitted)
            if len(clause_expanded) > len(fitted):
                warnings.append(f"第一章《{title}》已按更细粒度拆分，适配模板段落数")
            fitted = clause_expanded
            while len(fitted) < slot_count:
                idx = _find_best_split_index(fitted)
                if idx is None:
                    break
                left, right = _split_paragraph_for_template(fitted[idx], allow_clause_split=True)
                if not left or not right:
                    break
...
def _split_paragraph_for_template(text: str, *, allow_clause_split: bool = False) -> tuple[str, str]:
...
def _build_chapter1_batch_prompt(
    *,
    ...
    def _paragraph_requirement(spec: Dict[str, Any]) -> str:
        ...
        if key == "industry_trends":
            return "至少 8 段，每段 120-220 字，段落必须是完整陈述句，并尽量把技术迭代、产品演进、应用扩展、竞争格局、产业链协同和未来展望拆开写。"
        if key == "industry_environment":
